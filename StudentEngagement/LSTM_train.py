...
class EngagementModel(nn.Module):
    def __init__(self, input_size=512, hidden_size=64, output_size=1):  # 🔧 hidden size 변경
        super().__init__()
        self.bilstm = nn.LSTM(input_size, hidden_size, batch_first=True, bidirectional=True)
        self.attn = Attention(hidden_size)
        self.norm = nn.LayerNorm(hidden_size * 2)
        self.dropout = nn.Dropout(0.7)  # 🔧 Dropout 증가
...
def train():
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-4, weight_decay=1e-4)  # 🔧 weight_decay 추가
